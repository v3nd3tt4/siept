...
class CI_DB_active_record extends CI_DB_driver {
	...
	public function replace_batch($table = '', $set = NULL)
	{
		if ( ! is_null($set))
		{
			$this->set_insert_batch($set);
		...
		for ($i = 0, $total = count($this->ar_set); $i < $total; $i = $i + 100)
		{
			...
			$sql = $this->_replace_batch($this->_protect_identifiers($table, TRUE, NULL, FALSE), $this->ar_keys, array_slice($this->ar_set, $i, 100));
